...
class CrossTransformer(nn.Module):
    ...
    def forward(self, x0: torch.Tensor, x1: torch.Tensor) -> List[torch.Tensor]:
        ...
        if self.flash is not None:
            ...
            # 替换 einsum: "b h i d, b h j d -> b h i j"
            # 使用 matmul: (b, h, i, d) @ (b, h, d, j) -> (b, h, i, j)
            sim = torch.matmul(qk0, qk1.transpose(-2, -1))
            
            attn01 = F.softmax(sim, dim=-1)
            attn10 = F.softmax(sim.transpose(-2, -1).contiguous(), dim=-1)
            
            # 替换 einsum: "bhij, bhjd -> bhid"
            # 使用 matmul: (b, h, i, j) @ (b, h, j, d) -> (b, h, i, d)
            m0 = torch.matmul(attn01, v1)
            
            # 替换 einsum: "bhji, bhjd -> bhid"
            # attn10.transpose(-2, -1) 的形状是 (b, h, i, j)
            m1 = torch.matmul(attn10.transpose(-2, -1), v0)
...
class MatchAssignment(nn.Module):
    ...
    def forward(self, desc0: torch.Tensor, desc1: torch.Tensor):
        ...
        # 替换 einsum: "bmd,bnd->bmn"
        # 使用 matmul: (b, m, d) @ (b, d, n) -> (b, m, n)
