...
end2end_model_path = "output/rknn/sl_end2end_simplified_512_512_rk3588.rknn"
target = "rk3588"

end2end_model = RKNN(verbose=True)

GrayScale = True
# -------------------------
# 1. 加载模型
# -------------------------
ret = end2end_model.load_rknn(end2end_model_path)
if ret != 0:
    print(f'Load RKNN model "{end2end_model_path}" failed!')
    exit(ret)
...
# 2. 初始化运行时
# -------------------------
ret = end2end_model.init_runtime(target=target)
if ret != 0:
...
lg_outputs = end2end_model.inference(inputs=[infer_img1, infer_img2])


print("lightglue_model infer end !")
print(f"耗时: {time.perf_counter() - star_infer:.6f} 秒")
# lg_outputs[0] -> (1,257,257) FP16

kpts0 = lg_outputs[0][0]  
kpts1 = lg_outputs[1][0] 
scores = lg_outputs[2][0]  # (257,257)
